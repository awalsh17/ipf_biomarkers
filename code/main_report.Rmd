...
##_ Clear the current session, to avoid unintended side effects from persisting data structures. 
...
if(is.na(myseed)) { stop("Set variable myseed to a valid integer") }
...
knitr::opts_chunk$set(
  ##_ Echo controls output of code chunks in the knitr output.  FALSE hides the code chunks
  echo = (params$show_code == "inline"), # code display is controlled by the show_code parameter in the YAML block
  eval = TRUE,
  ##_ suppress extraneous, usually unwanted, messages in your knitr output
  ##_ You might want to set some of these to true when debugging
...
scale_colour_discrete <- function(...) { ggsci::scale_color_npg()}
  # function(...) {
  # scale_colour_manual(..., values = rep(brewer.pal(n=8,name="Dark2"),5), na.value="grey60")}
scale_fill_discrete <- function(...) { ggsci::scale_fill_npg() }
  # function(...) {
  # scale_fill_manual(..., values = brewer.pal(n=8,name="Dark2"), na.value="grey60")}
```
...
resid_dta <- data.frame(apply(adj_data, 2, 
                              function(x) 
...
# dta_clean <- dta
# names(dta_clean) <- make.names(names(dta_clean))
...
biol_table <- data.frame("Label" = c("SP-D", "SP-A1",
                         ...
                         "group" = c(rep("Epithelial Damage",6),
...
biol_groups <- lapply(biol_groups, function(x) symbol_mapping$clean_label[symbol_mapping$Label%in% x])
...
```{r}
vars_of_interest <- c("disease","current_rx_simple",
                      "visits","race",
...
table_one <- dta %>%
  ...
  mutate(visits = n())%>% 
  arrange(Visit) %>% 
  summarise("Treatment (first visit)" = current_rx_simple[1],
            ...
            .groups = "drop") %>% 
  mutate(`Current smoker` = factor(`Current smoker`, levels = c("No", "Yes")),
...
table_one %>% as_gt() %>% gt::gtsave(filename = here::here("results/table_one.png"))
```
...
protein_annot <- rowAnnotation(
  ...
  col = list(group = c("ECM Remodeling" = "#E64B35FF", 
...
all_tests <- lapply(names(dta)[c(1:26)], function(x)
  run_model(filter(dta,  plate==1), x) %>% anova() %>% broom::tidy()
)
names(all_tests) <- names(dta)[c(1:26)]
...
color_scale1 <- c("Control" = "gray",
                 "IPF" = "deepskyblue4")
shape_scale1 <- c("Control" = 19,
                 "IPF" = 17)
color_scale2 <- c(
                 "None" = "#005f73",
                 "Pirfenidone" = "#ee9b00",
...
all_tests <- lapply(names(dta)[c(1:26)], function(x)
  ...
  run_model(filter(dta, disease=="IPF"), x) %>% 
    emmeans(pairwise ~current_rx_simple) %>% .$contrasts %>% as.data.frame()
)
names(all_tests) <- names(dta)[c(1:26)]
...
plots <- lapply(1:length(biol_groups), function(x){
  ...
  make_plot(dta_adjusted,
    ...
    scale_color_manual(values = color_scale2) + 
    scale_shape_manual(values = shape_scale2) +
    labs(y = NULL, #"Adjusted Protein Conc.",
         color = "Treatment",
         shape = "Treatment",
...
plot_over_time <- function(data, patient.ids){
  ...
  summarise(min_val=min(value, na.rm = T), 
            ...
            .groups = "drop") 
  ff <- with(mylimits,
           data.frame(value=c(min_val,max_val),
                      variable=c(variable,variable)))
aplot <- data %>% 
  filter(Subject.ID %in% patient.ids) %>% 
  mutate(mos_from_visit1 = days_from_visit1/30.417,
         trans_from_visit1 = trans_from_visit1/30.417) %>% 
  tidyr::pivot_longer(plot_vars, names_to = "variable") %>% 
  select(Subject.ID, variable, value, t_label,
         trans_from_visit1, mos_from_visit1, current_rx_simple) %>% 
  group_by(Subject.ID, variable) %>% 
  mutate(transplant_y = approx(x = mos_from_visit1, 
                               y = value, 
                               xout = trans_from_visit1[1])$y) %>%
  ungroup() %>% 
...
dta_adjusted %>% 
  tidyr::pivot_longer(plot_vars, names_to = "variable") %>% 
  select(Subject.ID, variable, value,  disease,
...
dta_adjusted %>% 
  ...
  geom_point(aes(color = t_label), size =0.5,
...
dta %>% 
  filter(disease=="IPF") %>% 
  select(DLCO.ml_mmHg_min_perc_pred,
                        FEV1_L,
                        FEV1_perc_pred, 
                        FVC_L,
                        FVC_perc_pred) %>% 
...
```{r}
plot_vars <- c("DLCO.ml_mmHg_min_perc_pred",
               "FEV1_L","FVC_L",
               "FEV1_perc_pred",
               "FVC_perc_pred")
dta %>% 
  tidyr::pivot_longer(all_of(plot_vars), names_to = "variable") %>% 
  select(Subject.ID, variable, value, plate, visit_date, 
         current_rx_dose,
         current_rx_simple) %>% 
  filter(!is.na(value)) %>% 
  ...
  labs(title = paste("Clinical metrics by treatment"),
...
make_plot_cont <- function(my_data, 
  ...
  geom_point(aes(color = !!color_var), show.legend = F) + 
  geom_jitter(height = 0, width=0.2, aes(color = disease),
...
all_tests <- lapply(names(dta)[c(1:26)], function(x)
  run_model(filter(dta,  disease=="IPF"), x) %>% summary() %>% 
    .$coefficients %>% as.data.frame() %>% .["DLCO.ml_mmHg_min_perc_pred",]
)
names(all_tests) <- names(dta)[c(1:26)]
...
all_tests <- lapply(names(dta)[c(1:26)], function(x)
  run_model(filter(dta,  disease=="IPF"), x) %>% summary() %>% 
    .$coefficients %>% as.data.frame() %>% .["FVC_perc_pred",]
)
names(all_tests) <- names(dta)[c(1:26)]
...
all_tests <- lapply(names(dta)[c(1:26)], function(x)
  run_model(filter(dta,  disease=="IPF"), x) %>% summary() %>% 
    .$coefficients %>% as.data.frame() %>% .["FEV1_perc_pred",]
)
names(all_tests) <- names(dta)[c(1:26)]
...
aplot <- data %>% 
  ...
  labs(title = paste("Biomarkers for", patient.id),
       color = "Treatment",
       x = "Visit Date")+
  theme(axis.text.x = element_text(angle=45, hjust=1, size = 6)) + 
  geom_point(data=ff,x = NA)
...
plot_vars <- c("DLCO.ml_mmHg_min_perc_pred",
               "FEV1_perc_pred",
               "FVC_perc_pred")
bplot <- data %>% filter(Patient.ID == patient.id) %>% 
  tidyr::pivot_longer(plot_vars, names_to = "variable") %>% 
  select(Subject.ID, variable, value,  visit_date, current_rx_simple) %>% 
  filter(!is.na(value)) %>% 
  ...
  scale_y_continuous(limits=c(0,110)) + 
  labs(title = paste("Clinical metrics for", patient.id),
       color = "Treatment",
       x = "Visit Date")+
  theme(axis.text.x = element_text(angle=45, hjust=1))
list(a = aplot, b = bplot)
...
write.csv(dta, row.names = F,
          here::here('data/analysis_data_cleaned.csv'))
write.csv(dta_adjusted, row.names = F,
